 ...
 * Dashboard.java
 *
 * Main JFrame for the Admin role. Renders a left navigation sidebar and a
 * CardLayout content area that lazily instantiates each management panel on
 * first visit. Handles session validation via a background Swing Timer that
 * fires every 60 seconds; on invalidation it forces a logout. Exposes
 * stopForLogout() so settings panels can kill the timer before initiating
 * their own logout sequence, preventing a second LoginFrame from opening.
 ******************************************************************************/
...
public class Dashboard extends DashboardFrameBase {
    ...
    public void showAdminSettings() { showPanel("AdminSettings"); }
    ...
    public void showAndResetAdminSettings() {
        if (!panelMap.containsKey("AdminSettings")) showPanel("AdminSettings");
        else                                         cardLayout.show(contentPanel, "AdminSettings");
        AccountSettingsPanel p = (AccountSettingsPanel) panelMap.get("AdminSettings");
        if (p != null) p.showMainSettings();
    }
    ...
    public void onDataChanged() {
        ...
        if (overviewPanel instanceof RefreshablePanel) {
            ...
            lastRefreshTimes.put("Apercu", System.currentTimeMillis());
        }
    ...
    private void enforcePasswordChangeIfRequired() {
        if (currentUser != null && currentUser.isMustChangePassword()) {
            SwingUtilities.invokeLater(this::showAndResetAdminSettings);
        }
    }
    ...
    protected JPanel buildNavigationPanel() {
        ...
        if (currentUser != null && currentUser.getRole() == User.Role.Admin) {
            ...
            JButton btnDeptMgmt     = createNavButton("Facultés",               "Departements", "department.png");
            JButton btnBlocMgmt     = createNavButton("Départements",           "Blocs",        "bloc.png");
            JButton btnSchedule     = createNavButton("Visualiseur d'Horaires", "Horaires",     "schedule.png");

            panel.add(btnOverview,     "growx");
            panel.add(btnDeptMgmt,     "growx");
            panel.add(btnBlocMgmt,     "growx");
        ...
        btnLogout.addActionListener((ActionEvent e) -> {
            ...
            UIUtils.logout(Dashboard.this);
        });
        panel.add(btnLogout, "growx, gaptop 180");
    ...
    protected void buildPanelFactories(Map<String, Supplier<JPanel>> panelFactories) {
        if (currentUser != null && currentUser.getRole() == User.Role.Admin) {
            ...
            panelFactories.put("Reservations",  () -> new ReservationManagementPanel(this));
            panelFactories.put("Salles",        () -> new RoomManagementPanel(this));
            panelFactories.put("Niveaux",       () -> new NiveauManagementPanel(this));
            panelFactories.put("Utilisateurs",  () -> new UserManagementPanel(this));
            panelFactories.put("Departements",  () -> new DepartementManagementPanel(this));
            panelFactories.put("Blocs",         () -> new BlocManagementPanel(this));
            panelFactories.put("Horaires",      () -> new ScheduleViewerPanel(this));
            panelFactories.put("AdminSettings", () -> new AccountSettingsPanel(this, this::showAdminDashboard));
        }
    }
}
